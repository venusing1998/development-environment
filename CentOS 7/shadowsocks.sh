...
function pre_install(){
    ...
    read -p "(Default password: whsir):" shadowsockspwd
    if [ "$shadowsockspwd" = "" ]; then
        shadowsockspwd="blog.whsir.com"
    fi
...
function iptables_set(){
    ...
    if [ $? -eq 0 ]; then
        /etc/init.d/iptables status | grep '443' | grep 'ACCEPT' >/dev/null 2>&1
        if [ $? -ne 0 ]; then
            /sbin/iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport 443 -j ACCEPT
            /etc/init.d/iptables save
            /etc/init.d/iptables restart
        else
            echo "port 443 has been set up."
        fi
    else
...
function install_ss(){
    ...
    if [ -f /usr/bin/pip ]; then
        ...
        echo -e "Your Server Port: \033[41;37m 443 \033[0m"
        ...
        echo ""
        echo "pip install failed! Please visit blog.whsir.com and contact."
        exit 1
    fi
}
